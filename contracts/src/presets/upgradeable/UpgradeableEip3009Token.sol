...
/**
 * @title UpgradeableEip3009Token
 * @notice Eip3009Token behind a proxy: the same modules, plus an upgrade path that
 *         is scheduled, delayed and vetoable.
 *
 * @dev This contract is an implementation, not a token
 *
 *  It is deployed once and pointed at by an ERC-1967 proxy; the proxy is the
 *  address holders use. Storage written by an implementation's constructor lands
 *  in the implementation's own storage, where no proxy will ever read it, so
 *  name, symbol and the admin are set through `initializeToken` and
 *  `initializeAdmin` called on the proxy instead.
 *
 *  Decimals is the exception, and is a constructor argument for that reason: it
 *  is an immutable, which lives in code rather than storage and is therefore
 *  read by every proxy pointing here. See docs/deploying.md.
 *
 ...
 *  `UpgradeControl` adds a delay and a Guardian veto so the change is visible
 *  before it binds, but the admin key can eventually change any rule this token
 *  enforces.
 *
 *  The immutable `Eip3009Token` is the alternative, at the cost of being unable to
 *  fix a bug.
 */
contract UpgradeableEip3009Token is
    TokenBase,
    Eip3009,
    Eip2612,
    MinterControl,
