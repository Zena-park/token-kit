...
|---|---|---|---|
| `Eip3009` | payment | Signature transfers with random `bytes32` nonces; any caller submits and pays the gas. Both USDC entry-point forms, `(v, r, s)` and `bytes` | Three functions in two forms each, three type hashes and a nonce map in the token |
